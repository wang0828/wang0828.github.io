...
public partial class gsb : System.Web.UI.Page
{
    ...
    public void databind()
    {
        ...
        SqlCommand cmd = new SqlCommand("select * from gsb", conn);//访问数据库的SQL语句存到了cmd中
    ...
    protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        string bz = (GridView1.Rows[e.RowIndex].Cells[6].Controls[0] as TextBox).Text.Trim();

        string aaa = "update gsb set bz='" + bz + "' where yhID='" + (GridView1.DataKeys[e.RowIndex].Values[0]).ToString().Trim() + "'";
    ...
    {
        SqlConnection conn = new SqlConnection("SERVER=.;DATABASE=tsgl;PWD=;UID=sa;");
        string sc = "delete from gsb where yhID='" + (GridView1.Rows[e.RowIndex].Cells[0]).Text.Trim() + "'";
